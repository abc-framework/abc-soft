    ...
    public function setTpl($tplName, $tpl = true)
    {
        ...
        if (false === $tpl) {
            ...
            if (false === ($this->tpl = @file_get_contents($path))) {
                throw new \DomainException('AbcTemplate: '. sprintf(ABC_TPL_NOT_FOUND, $path, $path));
            }
        }  
    ...
    */
    protected function checkblock($errors)
    {
        ...
        foreach ($errors as $bname => $v) {
            throw new \DomainException('AbcTemplate: '. sprintf(ABC_TPL_INVALID_BLOCK, $bname, $bname));
        }
    }
